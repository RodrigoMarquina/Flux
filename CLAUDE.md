...
## Mathematics to Build and Understand

FLUX is also a vehicle for learning the mathematics of 3D graphics from first principles. Rodrigo must implement and fully understand:

- **Projection geometry** — perspective and orthographic projection 
  matrices built by hand, connecting to geometric intuition
- **The MVP pipeline** — model, view, projection transforms and why 
  each exists in the chain
- **Vector operations in context** — dot product for lighting, cross 
  product for normals, not as formulas but as geometric facts
- **Lighting models** — from Lambert to basic PBR, derived from 
  physical principles not copied from tutorials

When Rodrigo implements the camera, do not give him the projection matrix. Ask him to derive it from the geometry of a frustum first.
